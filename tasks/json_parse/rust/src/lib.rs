...
#[cfg(test)]
use parser::{parse_json_string_value, parse_json_number, parse_json_boolean};
...
pub extern "C" fn run_task(params_ptr: *mut c_void) -> u32 {
    let params = unsafe {
        std::slice::from_raw_parts(params_ptr as *const u32, 2)
    };
...
mod tests {
    ...
    fn test_json_serialization() {
        let records = vec![
            JsonRecord { id: 1, value: 123, flag: false, name: "a1".to_string() },
            JsonRecord { id: 2, value: -456, flag: true, name: "a2".to_string() },
        ];
    ...
    fn test_json_parsing() {
        ...
        let records_multi = parse_json_string(json_multi).expect("Failed to parse multi-record JSON");
        
        assert_eq!(records_multi.len(), 2);
        assert_eq!(records_multi[1].id, 2);
        assert_eq!(records_multi[1].value, -456);
        assert_eq!(records_multi[1].flag, true);
    ...
    fn test_json_roundtrip() {
        ...
        let parsed_records = parse_json_string(&json_string)
    ...
    fn test_hash_consistency() {
        let records1 = vec![
            JsonRecord { id: 1, value: 123, flag: true, name: "test".to_string() },
            JsonRecord { id: 2, value: -456, flag: false, name: "data".to_string() },
        ];
        ...
        let records3 = vec![
            JsonRecord { id: 1, value: 124, flag: true, name: "test".to_string() }, // Different value
        ];
    ...
    fn test_parse_utilities() {
        ...
        let mut pos = 0;
        let parsed_string = parse_json_string_value(&json_chars, &mut pos)
        ...
        let mut pos = 0;
        let parsed_num = parse_json_number(&num_chars, &mut pos)
        ...
        let mut pos = 0;
        let parsed_neg = parse_json_number(&neg_chars, &mut pos)
            .expect("Failed to parse negative number");
        assert_eq!(parsed_neg, -456);
        
        // Test boolean parsing
        let true_chars: Vec<char> = "true".chars().collect();
        let mut pos = 0;
        let parsed_true = parse_json_boolean(&true_chars, &mut pos)
            .expect("Failed to parse true");
        assert_eq!(parsed_true, true);
        
        let false_chars: Vec<char> = "false".chars().collect();
        let mut pos = 0;
        let parsed_false = parse_json_boolean(&false_chars, &mut pos)
    ...
    fn generate_reference_hashes() {
        use crate::reference::{generate_all_vectors, export_vectors_to_json};
