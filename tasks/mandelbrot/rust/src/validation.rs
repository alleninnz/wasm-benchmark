...
pub fn validate_parameters(params: &MandelbrotParams) -> bool {
    ...
    if params.width == 0 || params.height == 0 || 
       params.width > MAX_IMAGE_DIMENSION || params.height > MAX_IMAGE_DIMENSION {
        return false;
    }
...
mod tests {
    ...
    fn test_parameter_validation() {
        ...
        // Invalid dimensions
        let invalid_width = MandelbrotParams { width: 0, ..valid_params };
        assert!(!validate_parameters(&invalid_width));
        
        let invalid_height = MandelbrotParams { height: 0, ..valid_params };
        assert!(!validate_parameters(&invalid_height));
        
        // Invalid scale factor
        let invalid_scale = MandelbrotParams { scale_factor: 0.0, ..valid_params };
        assert!(!validate_parameters(&invalid_scale));
        
        // Invalid floating point values
        let invalid_center = MandelbrotParams { center_real: f64::NAN, ..valid_params };
