...
use crate::{generate_random_matrix, naive_triple_loop_multiply, fnv1a_hash_matrix, validate_parameters};
...
fn generate_small_matrix_tests() -> Vec<TestVector> {
    ...
    test_cases.into_iter().map(|(dim, seed, name, desc)| {
        let params = MatrixMulParams { dimension: dim, seed };
...
fn generate_edge_case_tests() -> Vec<TestVector> {
    let test_cases = vec![
        ...
        (16, u32::MAX, "edge_max_seed", "Matrix with maximum seed value"),
    ];
...
mod tests {
    ...
    fn test_generate_test_vectors() {
        ...
        assert!(vectors.len() >= 10, "Should generate at least 10 test vectors");
        
        // All vectors should have valid hashes
        for vector in &vectors {
            assert_ne!(vector.expected_hash, 0, "Hash should not be zero for valid params: {}", vector.name);
        }
        
        // Should have different categories
        let categories: std::collections::HashSet<_> = vectors.iter()
            .map(|v| &v.category)
            .collect();
        assert!(categories.len() >= 3, "Should have multiple test categories");
    }
    ...
    fn test_invalid_params_zero_hash() {
        let invalid_params = MatrixMulParams { dimension: 0, seed: 12345 };
    ...
    fn test_serializable_params_conversion() {
        let params = MatrixMulParams { dimension: 16, seed: 42 };
