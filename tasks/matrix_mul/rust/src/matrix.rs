...
pub fn naive_triple_loop_multiply(a: &Vec<Vec<f32>>, b: &Vec<Vec<f32>>, c: &mut Vec<Vec<f32>>) {
    let n = a.len();
...
pub fn matrices_approximately_equal(a: &Vec<Vec<f32>>, b: &Vec<Vec<f32>>, tolerance: f32) -> bool {
    if a.len() != b.len() {
        return false;
    }
...
mod tests {
    ...
    fn test_create_identity_matrix() {
        ...
        for i in 0..3 {
            for j in 0..3 {
                if i == j {
                    assert_eq!(identity[i][j], 1.0, "Diagonal elements should be 1.0");
                } else {
                    assert_eq!(identity[i][j], 0.0, "Off-diagonal elements should be 0.0");
                }
            }
        }
    }
    ...
    fn test_matrix_multiply_zero() {
        ...
        assert!(matrices_approximately_equal(&zero, &result, 1e-6),
            "A × 0 should equal 0");
    }
    ...
    fn test_matrices_approximately_equal() {
        ...
        let b = vec![
            vec![1.0000001, 2.0000001],
            vec![3.0000001, 4.0000001],
        ];
        
        assert!(matrices_approximately_equal(&a, &b, 1e-5),
            "Matrices with small differences should be approximately equal");
        
        assert!(!matrices_approximately_equal(&a, &b, 1e-8),
