...
class VisualizationGenerator:
    ...
    def create_performance_comparison_chart(
        ...
        rust_ci_lower = [comp.t_test.confidence_interval_lower + comp.rust_stats.mean
                        if comp.t_test.confidence_interval_lower < 0 else comp.rust_stats.mean - abs(comp.t_test.confidence_interval_lower)
                        for comp in comparisons]
        rust_ci_upper = [comp.t_test.confidence_interval_upper + comp.rust_stats.mean
                        if comp.t_test.confidence_interval_upper > 0 else comp.rust_stats.mean + abs(comp.t_test.confidence_interval_upper)
                        for comp in comparisons]

        # Error bars for confidence intervals
        rust_errors = [[mean - lower for mean, lower in zip(rust_means, rust_ci_lower)],
                      [upper - mean for mean, upper in zip(rust_means, rust_ci_upper)]]

        # TinyGo uses same CI logic
        tinygo_ci_lower = [comp.tinygo_stats.mean - abs(comp.t_test.confidence_interval_lower)
                          for comp in comparisons]
        tinygo_ci_upper = [comp.tinygo_stats.mean + abs(comp.t_test.confidence_interval_upper)
                          for comp in comparisons]

        tinygo_errors = [[mean - lower for mean, lower in zip(tinygo_means, tinygo_ci_lower)],
        ...
        rust_bars = ax.bar(x - width/2, rust_means, width,
                          yerr=rust_errors,
                          label='Rust',
        ...
        tinygo_bars = ax.bar(x + width/2, tinygo_means, width,
                            yerr=tinygo_errors,
                            label='TinyGo',
        ...
        for i, comp in enumerate(comparisons):
            if comp.t_test.is_significant:
                ...
                max_height = max(rust_means[i] + rust_errors[1][i],
                               tinygo_means[i] + tinygo_errors[1][i])
                ax.text(i, max_height * 1.05, sig_marker,
        ...
        ax.set_title('WebAssembly Performance Comparison: Rust vs TinyGo',
        ...
        legend_elements = [
            Rectangle((0,0),1,1, facecolor=self._language_colors['rust'], alpha=0.8, label='Rust'),
            Rectangle((0,0),1,1, facecolor=self._language_colors['tinygo'], alpha=0.8, label='TinyGo'),
        ...
        plt.tight_layout()
        plt.savefig(output_path, dpi=self.config.dpi_detailed,
                   format=self.config.output_format, bbox_inches='tight')
        plt.close()

        return output_path

    def create_effect_size_visualization(
        self, comparisons: List[ComparisonResult], output_path: str = "reports/plots/effect_sizes.png"
    ) -> str:
        """
